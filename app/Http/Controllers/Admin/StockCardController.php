...
class StockCardController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->filled('id_barang')) $query->where('id_barang', $request->id_barang);
        if ($request->filled('jenis_mutasi')) $query->where('jenis_mutasi', $request->jenis_mutasi);
        $stockCards = $query->latest('tanggal')->paginate(20);
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($v['id_batch'])) {
            ...
            if (in_array($v['jenis_mutasi'], ['masuk', 'retur'])) $batch->increment('sisa_stok', $v['jumlah']);
            else $batch->decrement('sisa_stok', $v['jumlah']);
        }
        $product->refresh();
        StockCard::create([
