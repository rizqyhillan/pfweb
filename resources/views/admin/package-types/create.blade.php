...
<div class="card"><div class="card-body">
  <form action="{{ route('admin.package-types.store') }}" method="POST">@csrf
    ...
    <div class="row mb-6">
      ...
      <div class="col-md-3"><label class="form-label">{{ request('section') === 'grooming' ? 'Harga Paket' : 'Harga/Malam' }} *</label><input type="number" step="0.01" name="harga_per_malam" class="form-control @error('harga_per_malam') is-invalid @enderror" value="{{ old('harga_per_malam', 0) }}" required />@error('harga_per_malam')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
      <div class="col-md-3"><label class="form-label">Keterangan</label><input type="text" name="description" class="form-control @error('description') is-invalid @enderror" value="{{ old('description') }}" />@error('description')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
    </div>
