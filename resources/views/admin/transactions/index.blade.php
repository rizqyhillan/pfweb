...
@section('content')
  <div class="d-flex justify-content-between align-items-center mb-6">
    ...
    <a href="{{ route('admin.transactions.create') }}" class="btn btn-primary"><i class="bx bx-plus me-1"></i> Transaksi Baru</a>
  </div>
        ...
        <tbody class="table-border-bottom-0">
          @forelse($transactions as $trx)
            <tr>
              ...
              <td>
                <div style="font-weight: 600; color: #333;">{{ $trx->pelanggan->nama ?? '-' }}</div>
                <div style="font-size: 12px; color: #888;">{{ $trx->kode_transaksi }}</div>
              </td>
              ...
              <td>
                <div class="dropdown">
                  ...
                  <div class="dropdown-menu">
                    <a class="dropdown-item" href="{{ route('admin.transactions.show', $trx) }}">
    ...
    if (typeof window.Echo !== 'undefined') {
      ...
      window.Echo.channel('transactions')
        .listen('.new-transaction', (e) => {
          ...
          let noDataTr = tbody.querySelector('td[colspan="8"]');
          if (noDataTr) noDataTr.parentElement.remove();

          let totalFormat = new Intl.NumberFormat('id-ID').format(trx.total);
          let dateStr = trx.tanggal ? new Date(trx.tanggal).toLocaleString('id-ID', {
          ...
          let html = `
            <tr style="animation: slideIn .3s ease;">
              ...
              <td>
                <div style="font-weight:600;color:#333;">${trx.pelanggan ? trx.pelanggan.nama : '-'}</div>
                <div style="font-size:12px;color:#888;">${trx.kode_transaksi}</div>
              </td>
