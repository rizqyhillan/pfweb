  ...
  <form action="{{ route('admin.medical-records.update', $medical_record) }}" method="POST" enctype="multipart/form-data">@csrf @method('PUT')
    ...
    <div class="row mb-6">
      <div class="col-md-6">
        ...
        <select class="form-select" name="id_dokter">
          ...
          @foreach($doctors as $d)
            <option value="{{ $d->id }}" {{ old('id_dokter', $medical_record->id_dokter) == $d->id ? 'selected' : '' }}>
              {{ $d->nama }}
            </option>
          @endforeach
        </select>
      ...
      <div class="col-md-3">
        ...
        <input type="number" step="0.01" class="form-control" name="berat_saat_itu" value="{{ old('berat_saat_itu', $medical_record->berat_saat_itu) }}" />
      </div>
      <div class="col-md-3">
        <label class="form-label">Tanggal *</label>
        <input type="datetime-local" class="form-control" name="tanggal" value="{{ old('tanggal', $medical_record->tanggal?->format('Y-m-d\TH:i')) }}" required />
      </div>
    </div>
    <div class="row mb-6">
    ...
    <div class="mb-6">
      <label class="form-label">Tambahkan Foto Baru (Bisa lebih dari 1)</label>
