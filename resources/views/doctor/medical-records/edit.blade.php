...
<div class="card"><div class="card-body">
  <form action="{{ route('doctor.medical-records.update', $medical_record) }}" method="POST">@csrf @method('PUT')
    <div class="row mb-6">
      ...
      <div class="col-md-3"><label class="form-label">Berat (kg)</label><input type="number" step="0.01" class="form-control" name="berat_saat_itu" value="{{ old('berat_saat_itu', $medical_record->berat_saat_itu) }}" /></div>
      <div class="col-md-3"><label class="form-label">Tanggal *</label><input type="datetime-local" class="form-control" name="tanggal" value="{{ old('tanggal', $medical_record->tanggal?->format('Y-m-d\TH:i')) }}" required /></div>
    </div>
    <div class="row mb-6">
